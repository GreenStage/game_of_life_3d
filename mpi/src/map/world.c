...
int * world_list_to_arr(int * arraySize,int x, int y){
  int a;
  int* tempBuff = (int*) malloc(sizeof(int) * world->size);
  int* retval;
  cell_ptr aux;
  *arraySize = 0;
  for(aux = world->cell_matrix[x][y], a = 0; aux != NULL; a++, aux = aux->next){
    temp_buff[a] = aux.pos.z;
  }
...
void world_init( int size){
  ...
  world->list_to_arr = world_list_to_arr;
